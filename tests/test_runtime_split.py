...
def test_azure_deploy_uses_oidc_and_gated_migrations():
    ...
    worker = infrastructure.split("resource analysisWorker ", 1)[1]
    assert "triggerType: 'Manual'" in worker
    assert "eventTriggerConfig" not in worker
    assert "poligrapher_app.cost_worker" in worker
    assert "poligrapher_app.cost_dispatcher" in infrastructure
