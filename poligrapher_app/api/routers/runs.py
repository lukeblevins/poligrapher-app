...
UPLOAD_METHODS = {"pdf_upload", "captured_text"}
...
def _persist_uploaded_policy(
    *,
    ...
    db.refresh(policy)
    try:
        policy.content_hash = file_hash(source_path)
        policy.source_blob_key = source_key(policy.id, filename)
        get_storage().upload_file(policy.source_blob_key, source_path, content_type="application/pdf")
        db.commit()
    except Exception:
        db.delete(policy)
        db.commit()
    ...
    task_title = (
        f"Analyze captured policy text for {provider.name}"
        if method == "captured_text"
        else f"Analyze PDF for {provider.name}"
    )
...
def _rerun_availability(policies: list[Policy]) -> RerunAvailability:
    ...
    storage = get_storage()
    upload = next((policy for policy in policies if policy.method in UPLOAD_METHODS), None)
    if upload:
...
def rerun(provider_id: uuid.UUID, run_id: uuid.UUID, request: Request, db: Db):
    ...
    registry = request.app.state.tasks
    upload = next((policy for policy in originals if policy.method in UPLOAD_METHODS), None)
    if upload:
        policy = Policy(
            provider_id=provider.id,
            url=upload.url,
...
async def upload_pdf(provider_id: uuid.UUID, request: Request, db: Db,
    ...
    with tempfile.NamedTemporaryFile(prefix="poligrapher-upload-", suffix=".pdf",
        ...
        while chunk := await pdf_file.read(1024 * 1024):
            upload.write(chunk)
        upload.flush()
        return _persist_uploaded_policy(
            provider=provider,
            request=request,
            db=db,
...
@router.post("/api/providers/{provider_id}/text-uploads", response_model=TaskStatus)
def upload_captured_text(
    provider_id: uuid.UUID,
    body: CapturedPolicyText,
    request: Request,
    db: Db,
):
    """Normalize browser-rendered policy text and queue the standard PDF pipeline."""
    from poligrapher_app.services.policy_documents import render_captured_policy_pdf

    provider = _provider_or_404(provider_id, db)
    day = body.capture_date or date.today()
    filename = f"captured-policy-{day.isoformat()}.pdf"
    temp_root = os.getenv("TEMP_WORKSPACE_ROOT") or None
    with tempfile.NamedTemporaryFile(prefix="poligrapher-captured-", suffix=".pdf",
                                     dir=temp_root) as upload:
